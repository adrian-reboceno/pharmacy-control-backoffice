...
@Component({
  ...
  template: `
    <div class="dialog-header">
      <h2>{{ isEdit ? 'Editar vía de administración' : 'Nueva vía de administración' }}</h2>
      ...
      <div mat-dialog-content class="dialog-content">
        ...
        <mat-form-field appearance="outline" class="full-width" subscriptSizing="dynamic">
          ...
          <input matInput formControlName="code" maxlength="10"
                 placeholder="ej. VO" />
          <mat-hint>Código estándar (máx. 10 caracteres)</mat-hint>
        ...
        <mat-form-field appearance="outline" class="full-width" subscriptSizing="dynamic">
          ...
          <textarea matInput formControlName="description" rows="3"
                    maxlength="500"
                    placeholder="ej. Administración por la boca."></textarea>
      ...
      <div mat-dialog-actions align="end" class="dialog-actions">
        ...
        <button type="submit" mat-flat-button color="primary"
          ...
          @if (saving()) {
            ...
            {{ isEdit ? 'Guardar cambios' : 'Crear vía' }}
          }
        </button>
      </div>
    </form>
...
export class RouteFormDialogComponent implements OnInit {
  ...
  data: RouteFormDialogData = inject(MAT_DIALOG_DATA);
  ...
  ngOnInit() {
    this.isEdit = this.data.route !== null;
    if (this.isEdit && this.data.route) {
      this.form.patchValue({
        name:        this.data.route.name,
        code:        this.data.route.code,
  ...
  submit() {
    ...
    const request = this.isEdit && this.data.route
      ? this.service.update(this.data.route.id, command)
